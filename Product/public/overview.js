...
async function trelloActionsUsersBoards() {
  ...
  Actions.forEach((i) => {
    switch (i.object.type) {
      ...
      case "updateCard":
        if (i.object.data.card.cover != undefined) {
          ...
          i.userMessage =
            "This was updated " +
            i.object.data.board.updateCheckItemStateOnCard; //Dette skal rettes. Dávur.
          break;
        }
        break;
      ...
      case "copyBoard":
        i.userMessage =
          'created a new board from template "' +
    ...
    if (i.userMessage == undefined) {
      i.userMessage =
        "json: " +
        JSON.stringify(i.object.data) +
        JSON.stringify(i.object.type);
    } else {
      i.userMessage =
        i.date +
        " (Trello) " +
