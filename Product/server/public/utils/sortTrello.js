  ...
  switch (i.object.type) {
    ...
    case "addToOrganizationBoard":
      i.message =
        ...
        i.object.data.organization.name +
        '" to the board "' +
        i.object.data.board.name +
        '"';
      break;
    case "moveCardFromBoard":
      ...
      let responseTarget = await fetch(
        `https://api.trello.com/1/boards/${boardTarget}/?key=0b862279af0ae326479a419925f3ea7a&token=${window.sessionStorage.getItem(
          "trello-token"
        )}`
      );
    ...
    case "moveCardToBoard":
      ...
      let responseSource = await fetch(
        `https://api.trello.com/1/boards/${boardSource}/?key=0b862279af0ae326479a419925f3ea7a&token=${window.sessionStorage.getItem(
          "trello-token"
        )}`
      );
      ...
      i.message =
        ...
        '")';
      console.log(
        "This is doubled, check the code and the HTML for date=",
        i.date
      );
      break;
    case "commentCard":
    ...
    case "updateBoard":
      if (i.object.data.old.name) {
        ...
        if (i.object.data.old.prefs.background) {
          i.message =
            'Changed background on board "' + i.object.data.board.name + '"';
        } else if (i.object.data.old.prefs.permissionLevel) {
          i.message =
            "Changed permission level to " +
            i.object.data.board.prefs.permissionLevel +
            ' on board "' +
            i.object.data.board.name +
            '"';
        }
    ...
    case "updateCard":
      if (i.object.data.card.cover != undefined) {
        ...
        i.message =
          'Deadline marked as accomplished on card: "' +
          i.object.data.card.name +
          '"';
      } else if (i.object.data.old.dueComplete) {
        i.message =
          'Deadline unmarked as accomplished on card: "' +
          i.object.data.card.name +
          '"';
    ...
    case "addMemberToBoard":
      i.message =
        `The member was added to the board "` + i.object.data.board.name + '"';
      break;
    ...
    case "updateCheckItemStateOnCard":
      if (i.object.data.checkItem.state == "complete") {
        i.message =
          'Marked as complete: The Checkbox Item "' +
          i.object.data.checkItem.name +
          '" (Card: "' +
          i.object.data.card.name +
          '")';
      } else if (i.object.data.checkItem.state == "incomplete") {
        i.message =
          'Marked as incomplete: The Checkbox Item "' +
          i.object.data.checkItem.name +
          '" (Card: "' +
          i.object.data.card.name +
          '")';
